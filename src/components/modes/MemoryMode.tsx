...
import { Eye, RotateCcw, Trophy, Check, Image as ImageIcon, BookOpen } from 'lucide-react';
...
export const MemoryMode: React.FC<Props> = ({ words, showPictures: initialPictures = false, onTogglePictures }) => {
  ...
  const initGame = () => {
    ...
    chosenWords.forEach(w => {
      ...
      list.push({ id: `def_${w.id}`, wordId: w.id, type: 'def', text: w.definition, imageUrl: imgUrl });
    });
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mb-6 px-1">
        <div className="flex items-center gap-4 text-slate-700 font-semibold text-sm">
        ...
        <div className="flex items-center bg-slate-100 p-1 rounded-xl border border-slate-200 text-xs font-semibold">
          ...
          <button
            ...
            title="Match with definition text"
          >
          ...
          <button
            ...
            title="Match with picture cards"
          >
          ...
          return (
            ...
            >
              ...
              >
                ...
                {/* Front (Revealed) */}
                <div
                  className={`absolute inset-0 w-full h-full rounded-xl p-2 flex flex-col justify-center items-center text-center shadow-md border-2 backface-hidden select-none overflow-hidden ${
                    isMatched
                ...
                >
                  {card.type === 'term' ? (
                    <span className="font-bold text-sm sm:text-base text-indigo-900 leading-tight">
                      {card.text}
                    </span>
                  ) : isPicturesMode && card.imageUrl ? (
                    ...
                    <span className="text-xs leading-snug line-clamp-4 text-slate-600">
                      {card.text}
                    </span>
                  )}
                  {isMatched && (
                    <span className="absolute bottom-1 bg-white/90 backdrop-blur-xs px-2 py-0.5 rounded-full flex items-center text-[10px] text-emerald-700 font-bold shadow-2xs border border-emerald-200">
